...
1. Analysis on the data aggregated by average behavior score (d.a and d.af), in order to replicate the analysis done in the original research.
...
```{r}
# d.a is the aggregated data with speakerSex and plev as numeric, while d.af codes speakerSex and plev as factors
...
```{r}
boxplot(behaviorScore ~ factor(plev, levels = c('Low', 'High')), data = d.af, 
        xlab="Hierarchy Condition", ylab = "Behavior Score", border = c("red", "blue"),
        main = "Behavior Score between Hierarchy Condition")
boxplot(behaviorScore ~ factor(speakerSex, levels = c('Male', 'Female')), data = d.af,
        xlab="Speaker Sex", ylab = "Behavior Score", border = c("red", "blue"),
        main = "Behavior Score between Speaker Sex")
```

#### Geometric Plots

By plotting behavior score against hierarchy condition and sex, we will look for potential main effects and interaction effects that we are interested in testing with our anova.

```{r}

ggplot(d.a, aes(x = plev, y = behaviorScore, col = factor(speakerSex, levels = c(0, 1), labels=c("Male", "Female")))) +
...
ggplot(d.a, aes(x = plev, y = behaviorScore, col = factor(speakerSex, levels = c(0, 1), labels=c("Male", "Female")))) +
  geom_smooth(method = loess.smooth, se = F) + 
  geom_point() + 
  xlab("Hierarchy Condition") + 
  ylab("Behavior Score") +
  scale_colour_discrete("Speaker Sex")
...
Just like with the aggregated data, we will gather an idea of the distribution of behavior scores in relation to hierarchy condition and speaker sex.

```{r}
boxplot(behaviorScore ~ factor(plev, levels = c('Low', 'High')), data = d.rawf, 
        xlab="Hierarchy Condition", ylab = "Behavior Score", border = c("red", "blue"),
        main = "Behavior Score between Hierarchy Condition")
boxplot(behaviorScore ~ factor(speakerSex, levels = c('Male', 'Female')), data = d.rawf,
        xlab="Speaker Sex", ylab = "Behavior Score", border = c("red", "blue"),
        main = "Behavior Score between Speaker Sex")
```
...
By plotting behavior score against hierarchy condition and sex, we will check to ensure that the effects found using the aggregated data are still present in the raw data.
